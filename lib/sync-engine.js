...
export async function push() {
  ...
  try {
    ...
    if (!isConfigured(settings)) {
      return { success: false, message: 'Extension not configured. Please open settings.' };
    }
    ...
    if (lastSyncData && bookmarksEqual(localData, JSON.parse(lastSyncData))) {
      return { success: true, message: 'No changes since last sync.' };
    }
      ...
      if (existing) {
        ...
        if (lastSyncData) {
          ...
          if (!bookmarksEqual(remoteData, lastSynced)) {
            ...
            return { success: false, message: 'Conflict detected: Remote was also modified.' };
          }
        }
      }
    }
    ...
    return { success: true, message: 'Bookmarks successfully pushed to GitHub.' };

  } catch (err) {
    if (err instanceof GitHubError && err.statusCode === 409) {
      await chrome.storage.local.set({ [STORAGE_KEYS.HAS_CONFLICT]: true });
      return { success: false, message: 'Conflict: The file was modified in the meantime.' };
    }
...
export async function pull() {
  ...
  try {
    ...
    if (!remoteFile) {
      return { success: false, message: 'No bookmarks found on GitHub.' };
    }
    ...
    if (bookmarksEqual(localData, remoteData)) {
      ...
      return { success: true, message: 'Bookmarks are already up to date.' };
    }
    ...
    return { success: true, message: 'Bookmarks successfully loaded from GitHub.' };
...
export async function sync() {
  ...
  try {
    ...
    if (!localChanged && !remoteChanged) {
      return { success: true, message: 'Everything in sync. No changes.' };
    }
    ...
    return {
      success: false,
      message: 'Conflict: Both local and remote bookmarks were modified. Please choose Push or Pull.',
    };
