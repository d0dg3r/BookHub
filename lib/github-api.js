...
export class GitHubAPI {
  ...
  async _fetch(url, options = {}) {
    ...
    if (response.status === 401) {
      throw new GitHubError('Ungültiges Token. Bitte prüfe deinen Personal Access Token.', 401);
    }
    if (response.status === 403) {
      const data = await response.json().catch(() => ({}));
      if (data.message && data.message.includes('rate limit')) {
        throw new GitHubError('GitHub API Rate Limit erreicht. Bitte warte etwas.', 403);
      }
      throw new GitHubError('Zugriff verweigert. Prüfe die Token-Berechtigungen.', 403);
    }
    if (response.status === 409) {
      throw new GitHubError('Konflikt: Die Datei wurde zwischenzeitlich geändert.', 409);
    }
  ...
  async getFile(path) {
    ...
    if (!response.ok) {
      throw new GitHubError(`Fehler beim Lesen der Datei: ${response.status}`, response.status);
    }
  ...
  async createOrUpdateFile(path, content, message, sha = null) {
    ...
    if (!response.ok) {
      ...
      throw new GitHubError(
        `Fehler beim Schreiben der Datei: ${errorData.message || response.status}`,
        response.status
      );
    }
  ...
  async getLatestCommitSha() {
    ...
    if (!response.ok) {
      throw new GitHubError(`Branch '${this.branch}' nicht gefunden.`, response.status);
    }
