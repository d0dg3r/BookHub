...
export class GitHubAPI {
  ...
  async _fetch(url, options = {}) {
    ...
    if (response.status === 401) {
      throw new GitHubError('Invalid token. Please check your Personal Access Token.', 401);
    }
    if (response.status === 403) {
      const data = await response.json().catch(() => ({}));
      if (data.message && data.message.includes('rate limit')) {
        throw new GitHubError('GitHub API rate limit exceeded. Please wait a moment.', 403);
      }
      throw new GitHubError('Access denied. Please check your token permissions.', 403);
    }
    if (response.status === 409) {
      throw new GitHubError('Conflict: The file was modified in the meantime.', 409);
    }
  ...
  async getFile(path) {
    ...
    if (!response.ok) {
      throw new GitHubError(`Error reading file: ${response.status}`, response.status);
    }
  ...
  async createOrUpdateFile(path, content, message, sha = null) {
    ...
    if (!response.ok) {
      ...
      throw new GitHubError(
        `Error writing file: ${errorData.message || response.status}`,
        response.status
      );
    }
  ...
  async getLatestCommitSha() {
    ...
    if (!response.ok) {
      throw new GitHubError(`Branch '${this.branch}' not found.`, response.status);
    }
